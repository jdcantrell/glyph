...
function set_image_height(els, targetHeight, maxWidth, currentWidth) {
  console.log('setting sizes for', els.length, maxWidth, currentWidth);
  _.each(els, function (el) {
    ...
    if (el.naturalHeight) {
      ...
      if (targetHeight > el.naturalHeight) {
        padding = (targetHeight - el.naturalHeight) / 2;
...
function layout_tiles($els, targetHeight, rowWidth, marginTotal) {
  ...
  $els.each(function (idx, el) {
    ...
    if (el.naturalHeight) {
      ...
      if (scaledWidth > el.naturalWidth) {
        scaledWidth = el.naturalWidth + 0;
      }
    ...
    if (currentWidth + scaledWidth <= rowWidth) {
      ...
      //update heights
      set_image_height(rowImgs, rowHeight, rowWidth - marginTotal, currentWidth);
      rowImgs = [];
      currentWidth = 0;
      if (scale_up) {
        rowImgs.push(el);
